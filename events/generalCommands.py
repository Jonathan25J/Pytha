...
class generalCommands(commands.Cog):
    ...
    async def slash(self, interaction: discord.Interaction) -> None:
        ...
        if isinstance(interaction.channel, discord.TextChannel):
            ...
            embed1.set_thumbnail(url=f'attachment://{self.icon.filename}')
            embed1.add_field(name="Pytha", value="Welcome I am Pytha", inline=False)
            await interaction.user.send(embed=embed1, file=self.icon)

        else:
            embed2 = discord.Embed(description=":)", color=0x2bb8d4)
            embed2.set_thumbnail(url=f'attachment://{self.icon.filename}')
            await interaction.response.send_message(embed=embed2, file=self.icon)
    ...
    async def start(self, interaction: discord.Interaction) -> None:
        if isinstance(interaction.channel, discord.TextChannel):
            ...
            view = startMenu()
            await interaction.response.send_message(embed=embed, view=view, ephemeral=True, file=thumbnail)
        ...
        if isinstance(interaction.channel, discord.TextChannel):
            ...
            for user in users:
                if user.username == str(interaction.user):
                    ...
                    await interaction.response.send_message(embed=embed, ephemeral=True, file=thumbnail)
                    break
            else:
                embed1 = discord.Embed(title="No user found",
                                       description="You don't have stored any stats, start the game with /start",
                                       color=0xa04b4b)
                embed1.set_author(name="Pytha-respond", icon_url=f'attachment://{self.icon.filename}')
                embed1.set_thumbnail(url=f'attachment://{self.icon.filename}')
                await interaction.response.send_message(embed=embed1, ephemeral=True, file=self.icon)
