...
async def getBoss(interaction):
    ...
    if result < 13.34:
        condition = 1
    embed = discord.Embed(color=boss_color[boss_level - 1])
    embed.set_image(url=f"{boss_url[boss_type - 1]}")
    embed.add_field(name=f"{boss_name[boss_type - 1]}", value=f"Level: {boss_level}", inline=False)
    embed.add_field(name="Strength", value=f"{boss_type}", inline=False)
    view = mechanics()
...
class mechanics(discord.ui.View):
    ...
    async def button1(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if condition == 0:
            lost = random.randint(0, 2) + boss_level
            ...
            won = random.randint(0, 2) + boss_level
